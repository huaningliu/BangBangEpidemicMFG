...
def rate_ODE_u(t, u, death, lambda_type, inter_alpha_s, inter_Z, inter_nu, beta, kappa, gamma, lambda_s, c_lambda, c_inf, c_dead, c_nu, n_blocks):
    Z  = inter_Z(t) 
    nu = inter_nu(t)
    alpha_s = inter_alpha_s(t)
    if lambda_type==2:
        lambda_s = lambda_s(t)
    # print(u[2*n_blocks:3*n_blocks])
    rate_u_S = beta * alpha_s * Z * (u[0:n_blocks]-u[n_blocks:2*n_blocks]) - c_lambda*((lambda_s-alpha_s)**2) + kappa*nu*(u[0:n_blocks]-u[2*n_blocks:3*n_blocks]) - c_nu*nu
...
def solver_HJB(death, lambda_type, inter_alpha_s, inter_Z, inter_nu, beta, kappa, gamma, lambda_s, c_lambda, c_inf, c_dead, c_nu, n_blocks, u_T, t_grid, T):
...
def stoch_block_check_original(n_blocks, n_states, Nt, lambda_s_in, lambda_i_in, lambda_r_in, \
                      graphon, beta, kappa, gamma, rho, c_lambda, c_inf, c_dead, c_nu, \
                      t_grid, T, p_0, u_T, n_print, exp_id, block_dens, lambda_type, lambda_duration, death, epsilon, V):
    ...
    if (lambda_type==0 or lambda_type==1):
        ...
        after_u = solver_HJB(death, lambda_type, inter_alpha_s, inter_Z, inter_nu, beta, kappa, gamma, lambda_s, c_lambda, c_inf, c_dead, c_nu, n_blocks, np.reshape(u_T,(n_blocks*n_states)), t_grid, T)
